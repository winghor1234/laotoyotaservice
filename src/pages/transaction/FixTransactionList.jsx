...
const FixTransactionList = () => {
    ...
    const FixTransactionDetail = async (id) => {
        try {
            const fixId = await axiosInstance.get(APIPath.SELECT_FIX_BY_BOOKING(id));
            navigate(`/user/transaction-fix-detail/${fixId?.data?.data?.fix_id}`);
        } catch (error) {
            console.log(error);
        }
                    ...
                    {fix?.filter((item) => item.bookingId !== null).map((item, index) => (
                        <div key={index} className="grid grid-cols-7 gap-2 md:gap-4 px-3 md:px-4 lg:px-6 py-3 md:py-4 items-center hover:bg-gray-50 transition-colors cursor-pointer text-xs md:text-sm lg:text-base">
                            ...
                            <div className="flex items-center justify-center">
                                <Eye onClick={() => FixTransactionDetail(item.booking.booking_id)} className="text-gray-600 h-4 md:w-5 md:h-5 hover:text-gray-800" />
                            </div>
                        </div>
                    ))}
                </div>
...
export default FixTransactionList;
