...
const AddAdmin = ({ show, onClose, handleFetch }) => {
  ...
  const {
    register, handleSubmit, errors, loading, submitForm,
    provinceOptions, districtOptions,
    selectedProvince, selectedDistrict,
    handleProvinceChange, handleDistrictChange,
  } = useAddAdminForm({ handleFetch, onClose });
    ...
    <>
      ...
      >
        ...
        <div className="flex flex-row items-center justify-between gap-2 sm:gap-3">
          ...
          <button className="bg-yellow-400 hover:bg-yellow-600 transition-colors w-auto px-3 sm:px-4 py-2 text-white rounded-lg font-medium text-sm sm:text-base">
            {t("import")}
          </button>
        </div>
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4 max-w-full w-full p-2 sm:p-3">
            ...
            {/* Password */}
            <div className="flex flex-col w-full">
              <label className="text-sm sm:text-base font-medium mb-1">{t("password")}</label>
              <input
                {...register("password")}
                type="password"
                className="w-full h-[42px] sm:h-[45px] rounded-lg text-sm sm:text-base border border-gray-300 outline-none px-3 hover:border-red-500 focus:border-red-500 transition-colors"
