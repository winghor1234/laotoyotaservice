...
import { useEditColorForm } from "../../component/schemaValidate/colorValidate/EditColorValidate";
import { useTranslation } from "react-i18next";

const EditColor = ({ show, onClose, colorId, fetchColor }) => {
    const { t } = useTranslation("color");
    const { register, handleSubmit, formState: { errors }, submitForm, loading } = useEditColorForm({ onClose, fetchColor, colorId });
    ...
    return (
        <>
            ...
            <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-50 w-full max-w-2xl bg-white rounded-2xl shadow-lg p-4 sm:p-6 text-sm transition-all">
                <h2 className="text-lg sm:text-xl font-bold text-center mb-4">{t("edit_color_title")}</h2>
                <form onSubmit={handleSubmit(submitForm)} className="space-y-3 sm:space-y-4">
                    <div className="flex flex-col ">
                        <div className="flex flex-col">
                            ...
                            />
                            <div className="h-6">
                                {errors.colorName && <span className="text-red-500 text-xs">{errors.colorName.message}</span>}
                            </div>
                        </div>
                    </div>
